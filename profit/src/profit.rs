...
use polars::{error::{ErrString, PolarsError}, frame::DataFrame, prelude::{LazyFileListReader, LazyFrame, RoundMode, col, lit, when}};
use std::io::Write;
 use polars::prelude::SerWriter;

pub fn with_summary(portfolio_csv:&PathBuf, output_directory:&PathBuf) ->Result<(), polars::prelude::PolarsError> {
...
fn write_net_profit_per_ticker(output:&mut File,net_profit_per_ticker:LazyFrame) ->Result<(), polars::prelude::PolarsError> {
...
fn write_summary(profit_summary_path:&mut File, summary:LazyFrame) ->Result<(), polars::prelude::PolarsError> {
...
fn _money(df: &DataFrame, column: &str) -> Result<f64, PolarsError> {
    ...
    let val = df
        ...
        .get(0)
        .ok_or(
            PolarsError::ComputeError(  ErrString::from(format!("Missing column {column} in summary")))
    )?;
    Ok(val)
}
...
fn net_profit(portfolio_csv:&PathBuf) ->Result<LazyFrame, polars::prelude::PolarsError> {
    let df = common::polars::default_lazy_reder(portfolio_csv)
    .finish()?;

    let round = 2;
    let mode = RoundMode::HalfToEven;
    
    let tax_df= df.clone().with_columns([
            (col("purchase_value") + col("buy_commision")).alias("cost_basis"),
            (col("sale_value") - col("sell_commision")).alias("net_proceeds"),
       (col("buy_commision")+ col("sell_commision"))
            .sum()
            .round(round, mode)
            .alias("commission_total"),
