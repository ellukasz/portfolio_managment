...
pub fn write(conf: &Conf) -> Result<PathBuf, polars::prelude::PolarsError> {
    ...
    let df = normalize_csv(trade_orders_csv.as_path(), conf)?;

    let trade_orders_normalized_file =
        common::file::file_name_with_suffix(trade_orders_csv, "normalized.csv")?;
...
fn normalize_csv(csv: &Path, conf: &Conf) -> Result<LazyFrame, polars::prelude::PolarsError> {
    ...
    let dataset = df
        ...
        .with_columns([
            ((col("purchase_value") + col("buy_commission")) / col("buy_quantity"))
                .round(ROUND, ROUND_MODE)
